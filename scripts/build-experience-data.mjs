...
// Beyond-Seoul sector: the 'beyond' culture page uses a city shape
// (region/desc/activities, no eyebrow/meta). Map region "<place> · <travel>" to
// eyebrow + meta so it fits the same experience card/drawer as every other sector.
function beyondSector() {
  const page = CULTURE_PAGES.find(p => p.id === 'beyond');
  if (!page) throw new Error('culture page not found: beyond');
  const out = [];
  for (const item of page.items) {
    const d = CULTURE_DETAILS[item.code];
    if (!d) { droppedCulture.push('beyond/' + item.code + ' (no CULTURE_DETAILS)'); continue; }
    const parts = String(item.region || '').split('·').map(s => s.trim());
    out.push(applyOverride({
      code: item.code,
      name: item.name,
      eyebrow: parts[0] || 'Beyond Seoul',
      meta: parts.slice(1).join(' · ') || item.region || '',
      image: item.image,
      folder: folderOf(item.image),
      gallery: d.photos || (d.photo ? [d.photo] : undefined),
      heroEyebrow: d.hero?.eyebrow || '',
...
const EXPECT = { Heritage: 12, Shop: 16, Famous: 12, Beyond: 6, Cuisine: 30 };
...
const header =
  ...
  `// can load it at runtime. Counts: Heritage ${counts.Heritage} · Shop ${counts.Shop} · Famous ${counts.Famous} · Beyond ${counts.Beyond} · Cuisine ${counts.Cuisine}.\n`;
